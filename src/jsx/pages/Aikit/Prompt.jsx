...
function Prompt() {
    ...
    "
                        ...
                        {/* <Accordion className="accordion accordion-with-icon aikitaccordion" defaultActiveKey="0">
                            {deleteItem.map((item, index) => (
                                <Accordion.Item eventKey={`${index}`} key={index} className='ai-header-alert p-0 border-0' >
                                    <Accordion.Header className="rounded-lg prompt-header p-0">
                                        <span className="me-1">{item.icon}</span>
                                        <span className="accordion-header-text p-0">{item.title}</span>
                                        <span className="remove-prompt" onClick={() => handleDelete(index)}>{SVGICON.closeredcircle}</span>
                                    </Accordion.Header>
